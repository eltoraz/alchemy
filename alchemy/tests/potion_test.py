...
def test_create():
   ...
   eff  = [{'effect': 'restore_health', 'magnitude': 250.0},
           {'effect': 'regen', 'magnitude': 10.0}]
   rec  = [{'element': 'Earth', 'min': 1.0, 'max': 5.0},
           {'element': 'Soul', 'min': 1.0, 'max': 5.0}]
...
# TODO: rewrite this test so it doesn't need to be rewritten every time
#       the relevant potion from the potions list gets tweaked
def test_get_matches():
    test_elements = {'Fire': 5.0, 'Lightning': 6.0}
    ...
    eq_(result.description,
        "Apply to your ankles (or your species' equivalent)")
    eq_(result.effects, [{'effect': 'haste', 'magnitude': 2.0}])
    eq_(result.recipe, [{'element': 'Fire', 'min': 5.0, 'max': 10.0},
                        {'element': 'Lightning', 'min': 6.0, 'max': 8.0}])

    multiple_matches = {'Fire': 8.0, 'Lightning': 6.0, 'Air': 2.0}
    ...
    concentration_mismatch = {'Fire': 11.0, 'Lightning': 2.0}
    result = alchemy.potion.get_matches(concentration_mismatch)
    eq_(result, [])

    non_matching_elements = {'Shadow': 50.0, 'Ice': 100.0, 'Void': 40.0}
